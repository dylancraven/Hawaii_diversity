...
datt<-filter(datt, SizeClass==5) %>%
  filter(., Native_Status=="alien")

#####
#qc #
#####

length(unique(datt$PlotID)) # 227 plots
length(unique(datt$SPP_CODE3A)) # 62 spp   
...
scen2_selplots<-read.csv("Cleaned_Data/Scen2_Aliens_SelPlots.csv", header=TRUE)
scen2_selplots<-select(scen2_selplots,  -Plot_Area)

######################
# Step 3: Join data  #
######################

datt_aliens<-left_join(scen2_selplots,datt, by=c('geo_entity2','PlotID'))
datt_aliens$Native_Status<-droplevels(datt_aliens$Native_Status)
# quick qc

qcc<-datt_aliens%>% group_by(geo_entity2, Iteration)%>% summarize(plotn=length(unique(PlotID)))

qcc2<-qcc%>% group_by(geo_entity2)%>% summarize(iter=length(unique(Iteration)))

unique(datt_aliens$Native_Status)
#####################################
# Step 4: estimate diversity stuff  #
#####################################

datt_aliens$geo_entity2<-factor(datt_aliens$geo_entity2, 
                               levels=c("Hawai'i Island","Kaua'i Island","Maui Nui","O'ahu Island"))

curveZ<-list(); orderZ<-list();rangeZ<-list();alphaZ<-list();betaZ<-list(); radZ<-list();

for(i in 1:100){
  
  togg<-filter(datt_aliens, Iteration==i)
  ...
  oo<-dim(datt3)[2]-1
  datt3<-melt(datt3,id.vars="geo_entity2",measure.vars=1:oo,variable.name="SPP_CODE3A",
              value.name="RelAbund")
  ...
  radd$geo_entity2<-factor(radd$geo_entity2,
                           levels=c("Hawai'i Island","Maui Nui","O'ahu Island","Kaua'i Island"))
  ...
  rangez<-unique(select(togg, geo_entity2, PlotID, MAT, MAP, AridInd, PET, Elev_m, 
                        SubstrateAge_range, Plot_Area))
  rangezz<-dplyr::summarize(group_by(rangez,geo_entity2),mean_MAT=mean(MAT),
                            min_MAT=min(MAT),max_MAT=max(MAT),
                            ...
                            mean_SubstrateAge=mean(SubstrateAge_range, na.rm=TRUE), 
                            min_SubstrateAge=min(SubstrateAge_range, na.rm=TRUE),
                            max_SubstrateAge=max(SubstrateAge_range, na.rm=TRUE),
  ...
  rangezz<-select(rangezz,geo_entity2, mean_MAP,r_MAP, mean_MAT, r_MAT, mean_PET, 
                  r_PET,mean_Arid, r_Arid, 
                  min_Elev, max_Elev, r_Elev, min_SubstrateAge, max_SubstrateAge,
                  r_SubstrateAge, mean_SubstrateAge, totPlotArea)
...
write.table(orders.tog,"Cleaned_Data/Scen2_Aliens_10plots_HillN.csv",sep=",",row.names=F)

write.table(curves.tog,"Cleaned_Data/Scen2_Aliens_10plots_curves_estimates.csv",sep=",",row.names=F)
...
write.table(envs,"Cleaned_Data/Scen2_Aliens_10plots_Envconditions_summarized.csv",sep=",",row.names=F)

write.table(beta.tog,"Cleaned_Data/Scen2_Aliens_10plots_BetaPIE.csv",sep=",",row.names=F)

write.table(rad.tog,"Cleaned_Data/Scen2_Aliens_10plots_RADs.csv",sep=",",row.names=F)