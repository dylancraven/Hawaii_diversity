...
for(i in 1:3)
{
  for(j in 1:2000)
  {
    ...
    d<- datt3 %>% group_by(geo_entity2,PlotID) %>% sample_n_groups(9,replace=F)
    d<-ungroup(d)
...
write.table(orders.tog,"Cleaned_Data/Scen2_Natives_9plots_HillN.csv",sep=",",row.names=F)

write.table(curves.tog,"Cleaned_Data/Scen2_Natives_9plots_curves_estimates.csv",sep=",",row.names=F)
...
write.table(envs,"Cleaned_Data/Scen2_Natives_9plots_Envconditions_summarized.csv",sep=",",row.names=F)

write.table(beta.tog,"Cleaned_Data/Scen2_Natives_9plots_BetaPIE.csv",sep=",",row.names=F)

write.table(rad.tog,"Cleaned_Data/Scen2_Natives_9plots_RADs.csv",sep=",",row.names=F)

write.csv(ka_plotarea,"Cleaned_Data/Scen2_Natives_Kauai_PlotArea_9plots.csv",row.names=F)
