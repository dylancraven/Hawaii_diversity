...
# Scenario: 'Het + Age'##
#######################################
# just naturalized aliens & natives   #
#######################################
# 10 plots                            #
#######################################
...
scen2_selplots<-read.csv("Cleaned_Data/Scen2_Total_SelPlots.csv", header=TRUE)
...
datt_total<-left_join(scen2_selplots,datt, by=c('geo_entity2','PlotID'))

# quick qc

qcc<-datt_total%>% group_by(geo_entity2, Iteration)%>% summarize(plotn=length(unique(PlotID)))

qcc2<-qcc%>% group_by(geo_entity2)%>% summarize(iter=length(unique(Iteration)))

unique(datt_total$Native_Status)
#####################################
# Step 4: estimate diversity stuff  #
#####################################

datt_total$geo_entity2<-factor(datt_total$geo_entity2, 
...
for(i in 1:100){
  ...
  togg<-filter(datt_total, Iteration==i)
...
write.table(orders.tog,"Cleaned_Data/Scen2_Total_10plots_HillN.csv",sep=",",row.names=F)

write.table(curves.tog,"Cleaned_Data/Scen2_Total_10plots_curves_estimates.csv",sep=",",row.names=F)
...
write.table(envs,"Cleaned_Data/Scen2_Total_10plots_Envconditions_summarized.csv",sep=",",row.names=F)

write.table(beta.tog,"Cleaned_Data/Scen2_Total_10plots_BetaPIE.csv",sep=",",row.names=F)

write.table(rad.tog,"Cleaned_Data/Scen2_Total_10plots_RADs.csv",sep=",",row.names=F)