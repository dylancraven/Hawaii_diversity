...
length(unique(dat3$iteration)) # 5405
...
dat33<-select(dat33, Iteration2, geo_entity2, PlotID, SPP_CODE3A, Abundance_ha, r_PET,totPlotArea)
...
Scen3_curves.tog<-do.call(rbind.data.frame,curveZ)

Scen3_orders.tog<-do.call(rbind.data.frame,orderZ)

Scen3_rangez.tog<-do.call(rbind.data.frame,rangeZ)

Scen3_beta.tog<-do.call(rbind.data.frame,betaZ)

Scen3_rad.tog<-do.call(rbind.data.frame,radZ)
