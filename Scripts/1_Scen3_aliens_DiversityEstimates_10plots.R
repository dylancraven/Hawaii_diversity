...
ka_plotarea<-read.csv("Cleaned_Data/Scen2_Aliens_Kauai_PlotArea.csv",header=T)
...
write.table(scen3_selplots,"Cleaned_Data/Scen3_Aliens_10plots_SimComms.csv",sep=",",row.names=F)
...
qcc1<-select(scen3_dat, Iteration, geo_entity2, PlotID, Plot_Area)%>%
  distinct(.)

qcc<-dplyr::summarize(group_by(qcc1, geo_entity2, Iteration), plotn=length(unique(PlotID)), PlotArea=sum(Plot_Area))
...
write.table(orders.tog,"Cleaned_Data/Scen3_Aliens_10plots_HillN.csv",sep=",",row.names=F)

write.table(curves.tog,"Cleaned_Data/Scen3_Aliens_10plots_curves_estimates.csv",sep=",",row.names=F)
...
write.table(envs,"Cleaned_Data/Scen3_Aliens_10plots_EnvConditions_summarized.csv",sep=",",row.names=F)

write.table(beta.tog,"Cleaned_Data/Scen3_Aliens_10plots_BetaPIE.csv",sep=",",row.names=F)

write.table(rad.tog,"Cleaned_Data/Scen3_Aliens_10plots_RAD.csv",sep=",",row.names=F)