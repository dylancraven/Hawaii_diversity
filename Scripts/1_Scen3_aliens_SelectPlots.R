...
for(i in 1:7500){
  ...
  if(ka_rangezz$r_Arid>ka_range$Prob50q_Arid_r) next
...
qc2<- ka_dist_Scen3 %>% group_by(geo_entity2) %>% 
  summarize(iters=length(unique(Iteration)), min_dist=min(min_dist_km),
            ...
            min_Elev=min(r_Elev),max_Elev=max(r_Elev),
...
for(i in 1:7500){
  ...
  if(ma_rangezz$r_Arid>maui_range$Prob50q_Arid_r) next
...
for(i in 1:7500){
  ...
  if(oa_rangezz$r_Arid>oah_range$Prob50q_Arid_r) next
...
for(i in 1:7500){
  ...
  if(ha_rangezz$r_Arid>haw_range$Prob50q_Arid_r) next
...
write.csv(scen3_selplots, "Cleaned_Data/Scen3_Aliens_SelPlots.csv",row.names=F)
