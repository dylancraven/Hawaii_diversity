...
ka_plotarea<-read.csv("Cleaned_Data/Scen2_Natives_Kauai_PlotArea.csv",header=T)
...
for(i in 1:4){
  for(j in 1:10000){
    ...
    togg<- datt3 %>% group_by(geo_entity2,PlotID) %>% sample_n_groups(7,replace=F)
    togg<-ungroup(togg)
...
write.table(scen3_selplots,"Cleaned_Data/Scen3_Natives_7plots_SimComms.csv",sep=",",row.names=F)

#sscen3_selplots<-read.csv("Cleaned_Data/Scen3_Natives_7plots_SimComms.csv",header=T)
