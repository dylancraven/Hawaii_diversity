...
summarise(group_by(datt, geo_entity2), PlotArea=mean(Plot_Area), PlotN=length(unique(PlotID)))
...
qc<-dplyr::summarize(group_by(outt, geo_entity2 , Iteration), plotn=length(unique(PlotID)), PlotArea=sum(Plot_Area))

qc2<-dplyr::summarize(group_by(qc, geo_entity2), iter=length(unique(Iteration)), PlotAreaa=mean(PlotArea))
...
write.csv(ka_plotarea,"Cleaned_Data/Scen2_Total_Kauai_PlotArea.csv",row.names=F)
