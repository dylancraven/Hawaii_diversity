...
# Kauai has fewest plots (22)
# selected plots ( 10 plots per island, min dist. >0.1 km)

scen3_selplots<-read.csv("Cleaned_Data/Scen3_Total_SelPlots.csv", header=TRUE)
...
write.table(orders.tog,"Cleaned_Data/Scen3_Total_10plots_HillN.csv",sep=",",row.names=F)

write.table(curves.tog,"Cleaned_Data/Scen3_Total_10plots_curves_estimates.csv",sep=",",row.names=F)
...
envs<-dplyr::summarize(group_by(rangez.tog, geo_entity2), mean_MAP=mean(mean_MAP),r_MAP=mean(r_MAP),
                       ...
                       mean_minElev=mean(min_Elev),mean_maxElev=mean(max_Elev),r_Elev=mean(r_Elev),
                       m_PlotArea=mean(totPlotArea),sd_PlotArea=sd(totPlotArea))

write.table(envs,"Cleaned_Data/Scen3_Total_10plots_EnvConditions_summarized.csv",sep=",",row.names=F)

write.table(beta.tog,"Cleaned_Data/Scen3_Total_10plots_BetaPIE.csv",sep=",",row.names=F)

write.table(rad.tog,"Cleaned_Data//Scen3_Total_10plots_RAD.csv",sep=",",row.names=F)