...
Beta_2<-read.csv("Cleaned_Data/Scen2_Natives_5plots_BetaPIE.csv",sep=",",header=T)
...
Beta_22<-filter(Beta_2, index=="beta_S_PIE")

Beta_22<-dplyr::summarize(dplyr::group_by(Beta_22, geo_entity2, Iteration),beta_ENS_PIE=mean(value))
...
Beta_33<-filter(Beta_3, index=="beta_S_PIE")

Beta_33<-dplyr::summarize(dplyr::group_by(Beta_33, geo_entity2, Iteration),beta_ENS_PIE=mean(value))
...
Beta_222<-filter(Beta_2, index=="beta_S")
Beta_222<-dplyr::summarize(dplyr::group_by(Beta_222, geo_entity2, Iteration),beta_S=mean(value))
...
Beta_Scen2s<-ggplot(data=Beta_222, aes(x=geo_entity2,y=beta_S,group=geo_entity2,color=geo_entity2))+
  ...
  scale_y_continuous(limits=c(0,15.8),breaks=c(0,5,10,15))+
  # scale_color_d3(palette="category20c")+
  scale_colour_manual(values=c("#d7191c","#fdae61","#abd9e9","#2c7bb6"))+
...
Beta_333<-filter(Beta_3, index=="beta_S")
Beta_333<-dplyr::summarize(dplyr::group_by(Beta_333, geo_entity2, Iteration),beta_S=mean(value))
...
png(filename="Figures/Fig5_Beta_ENSpie_S_natives_5plots.png", 
    units="in", 
    width=8, 
    height=6, 
    pointsize=2, 
